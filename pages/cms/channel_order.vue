 <template>
	<view class="margin-center padding-bottom-30">
		...
		<view v-if="orderList.length > 0">
			<view v-for="(item,index) in orderList" :key="index" @tap="handleDetail(item.order_token)" class="bg-color-white padding-20 border-radius-15 margin-top-30">
				<view class="d-row d-jc-between padding-10 font30 padding-bottom-10">
					<view class="font28">
						<text class="margin-right-20">
							<text class="font-bold">{{item.goods_name}}</text> 
							等<text class="font-bold font-30" style="color: red;">{{item.goods_num}}</text>件商品
						</text>
					
					</view>
					<view class="padding-left-10">¥ <text class="font30">{{item.total_rmbs}}</text></view>
				</view>
				<view class="d-row d-jc-between d-ai-centen padding-10 padding-top-30">
					<view class="flex-1 gray3">
						<text>{{item.status_fmt}}</text>
					</view>
					
					<view v-if="item.status == 0" class="flex-1 text-right">
						<!-- <button class="custom-style1" @tap.stop="cancelOrder('pay')">
							取消订单
						</button> -->
						<button class="custom-style" @tap.stop="handleDetail(item.order_token)">
							去支付
						</button> 
					</view>
					<view v-if="item.status == 2" class="flex-1 text-right">
	...
	export default {
		...
		methods: {
			getorderlist(data, callback = () => {}){
				this.$store.dispatch('shops/companyorder_list',data).then((res) => {
					if(res.code == 0) {
			...
			handleDetail(order_token){
				uni.navigateTo({
					url: '/pages/cms/channel_detail?order_token='+order_token,
